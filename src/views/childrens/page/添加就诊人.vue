<template>
	<div class="基本信息">
		<nut-navbar @on-click-back="this.$common.back" :leftShow="true" :rightShow="false">
			添加就诊人</nut-navbar>
		<label>
			<span>与本人关系</span>
			<div id="sex" @click="isVisible1 = true">
				{{ relationship || "请选择关系" }}
			</div>
		</label>
		<label>
			<span>姓名</span>
			<div>
				<input type="text" placeholder="1-10个字" v-model="name" />
			</div>
		</label>
		<label>
			<span>身份证号码</span>
			<div>
				<input type="text" placeholder="请输入您的身份证号码" v-model="IDcode" />
			</div>
		</label>
		<label>
			<span>手机号码(中国大陆+86)</span>
			<div>
				<input type="text" placeholder="请填真实的号码，方便我们联系" v-model="telephone" />
			</div>
		</label>
		<p class="tips">温馨提示：绑定的信息将作为就诊参考凭证</p>

		<div class="协议">
			<nut-checkbox v-model="selected" size="small"> </nut-checkbox>
			<p>我已阅读并同意《<router-link to="">用户协议</router-link>》</p>
		</div>

		<btn @click-btn="over" />
		<nut-picker :is-visible="isVisible1" :list-data="listData1" @close="switchPicker('isVisible1')" @confirm="setValue1"></nut-picker>
	</div>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			setValue1(chooseData) {
				for (let i in this.listData1[0]) {
					if (chooseData[0] == this.listData1[0][i]) {
						this.relationship=chooseData[0],
						this.relationshipIndex=parseInt(i)
					}
				}
			},
			...
			over() {
				...
				that.$post("/users/patient/addPatient", {
