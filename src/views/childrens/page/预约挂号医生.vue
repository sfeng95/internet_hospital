<template>
  <div class="xzys">
    <nut-navbar @on-click-back="$common.back"
                :rightShow="false">选择医生</nut-navbar>
    <div class="时间条">
      <div>
        <p @click="change(0)"
           :class="select_index === 0 ? 'selected' : ''">
          <span>不限</span>
          <br />
          <span>日期</span>
        </p>
        <p v-for="(item, index) in schedule"
           :key="index"
           @click="change(index + 1, item.timestamp)"
           :class="select_index === index + 1 ? 'selected' : ''">
          <span>{{ item.date }}</span>
          <br />
          <span>{{ item.week }}</span>
        </p>
